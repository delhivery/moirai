...
Wrapper::Wrapper(Solver *solverPtr, consumer_base_t::queue_t *sQPtr,
                 producer_base_t::queue_t *lQPtr, size_t batchSize)
    : mSolverPtr(solverPtr), Consumer(sQPtr, batchSize),
      Producer(lQPtr, batchSize) {}

Wrapper::Wrapper(const Wrapper &other)
    : Consumer(other), Producer(other), mSolverPtr(other.mSolverPtr) {}

Wrapper::Wrapper(Solver *solverPtr, producer_base_t::queue_t *sQPtr,
                 consumer_base_t::queue_t *lQPtr, size_t batchSize
...
#else
                 ,
                 std::string &node_uri, std::string &node_idx,
                 std::string &node_user, std::string &node_pass
#endif
...
#else
                 ,
                 std::string &edge_uri, std::string &edge_auth
#endif
                 )
    : mSolverPtr(solverPtr), Producer(lQPtr, batchSize),
...
#else
      ,
      node_uri(node_uri), node_idx(node_idx), node_user(node_user),
      node_pass(node_pass)
#endif
#ifdef WITH_EDGE_FILE
      ,
...
#else
      ,
      edge_uri(edge_uri), edge_auth(edge_auth)
#endif
{
  init_nodes();
  init_edges();
...
auto Wrapper::logger() const -> Poco::Logger & {
  return Poco::Logger::get("solver-wapper");
}

void Wrapper::stop(bool stop) {
  ::Consumer::stop(stop);
  ::Producer::stop(stop);
}

void Wrapper::init_nodes() {
  ...
  std::for_each(
      ...
      [this, &colocatedNodes](auto const &facilityWithSource) {
        ...
        auto processingTimeString = facilityAttrs["OutboundProcessingTime"];
        ...
        auto node = std::make_shared<TransportCenter>(
            facility["facility_code"], fname, cutoffDefault,
            MovementType::LINEHAUL, ProcessType::OUTBOUND, outProcessTime);
...
auto parse_route(const nlohmann::json &routeData) {
  ...
  for (size_t idxS = 0; idxS < eStop.size(); ++idxS) {
    for (size_t idxT = idxS + 1; idxT < eStop.size(); ++idxT) {
      ...
      subRoutes.emplace_back(
          fmt::format("{}.{}", eUUID, base(idxS, idxT, eStop.size())), eName,
          eType == "air" ? VehicleType::AIR : VehicleType::SURFACE,
          eType == "carting" ? MovementType::CARTING : MovementType::LINEHAUL,
          eOut, eDep, eDur, eInb, eDays, sNodeId, tNodeId);
    }
  }
  return subRoutes;
}

void Wrapper::load_edges(const nlohmann::json &data) {
    ...
    for (const auto &subRoute : subRoutes) {
      ...
      if (hasSNode and hasTNode) {
        ...
        auto edge = std::make_shared<TransportEdge>(
            subRoute.code(), subRoute.name(), subRoute.vehicle(),
            subRoute.movement(),
            sNodeAttr->latency(subRoute.movement(), ProcessType::OUTBOUND),
            tNodeAttr->latency(subRoute.movement(), ProcessType::INBOUND),
            subRoute.out_source(), subRoute.departure(), subRoute.duration(),
            subRoute.in_target(), subRoute.working_days());
...
auto Wrapper::find_paths(const std::string &item,
                         const std::string &item_source_idx,
                         const std::string &item_target_idx,
                         const datetime item_start,
                         const datetime item_target_limit, auto &subItems) const
    -> nlohmann::json {
  datetime mustReachBy = item_target_limit;
  nlohmann::json response = {
      {"_id", item}, {"waybill", item}, {"package", item}};
            ...
            std::views::transform([&tNodeDesc, this](const auto &subItem) {
              return mSolverPtr
                  ->find_path<PathTraversalMode::REVERSE, VehicleType::SURFACE>(
                      mSolverPtr->add_node(subItem.target()).first, tNodeDesc,
                      subItem.reach_by());
            }) |
            std::views::filter(
                [](auto const &cSubRoute) { return not cSubRoute.empty(); });
        auto cSubRoute =
            *(std::min_element(cSubRoutes.begin(), cSubRoutes.end(),
