...
uint16_t
TransportationEdge::time_departure(const uint16_t offset) const
{
  return (time_arrival_source - offset) % MINUTES_DIURNAL;
}

uint16_t
TransportationEdge::weight(
  ...
  const uint16_t offset) const
{
  ...
  auto time_minutes =
    std::chrono::duration_cast<std::chrono::minutes>(time_start.to_duration());
...
std::pair<detail::Edge, bool>
BaseGraph::add_edge(const detail::Vertex source,
                    const detail::Vertex target,
