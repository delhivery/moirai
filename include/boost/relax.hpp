...
bool
relax(typename graph_traits<Graph>::edge_descriptor e,
      const Graph& g,
      const WeightMap& w,
      PredecessorMap& p,
      DistanceMap& d,
...
template<class Graph,
         ...
         class BinaryPredicate>
bool
relax_target(typename graph_traits<Graph>::edge_descriptor e,
             const Graph& g,
             const WeightMap& w,
             PredecessorMap& p,
             DistanceMap& d,
             ...
             const BinaryPredicate& compare)
{
  ...
  /*
  std::cout << fmt::format(
    "Trying to go from {}@{} to {}@{} using {}. New distance@{}: {}",
    g[u]->name,
    date::format("%F %T", d_u),
    g[v]->name,
    date::format("%F %T", d_v),
    g[e]->code,
    g[v]->name,
    date::format("%F %T", combine(d_u, w_e)));
  */
  if (compare(combine(d_u, w_e), d_v)) {
    put(d, v, combine(d_u, w_e));
